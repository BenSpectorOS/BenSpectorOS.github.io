...
- [Circularity](#circularity)
  ...
  - [Lesson Steps](#lesson-steps)
    ...
    - [Animate Our Circles](#animate-our-circles)
      ...
      - [TODO 6 : Call game.checkCircleBounds on each of your circles](#todo-6--call-game.checkCircleBounds-on-each-of-your-circles)
...
We have created a function for you called `game.checkCircleBounds`. The function accepts a circle as an argument and uses a series of conditional statements to determine if the circle is within the bounds of the `canvas`. If a circle leaves the bounds of the screen it will appear on the opposite side of where it exited. So, if a circle leaves the `canvas` along the _bottom_ border, we need to place the circle fully off the `canvas` at the top border.
...
#### TODO 6 : Call game.checkCircleBounds on each of your circles

Great! Now that we have a function which will keep a circle within the screen, let's use it to continuously check our 5 circles. Within the `update` function call the `game.checkCircleBounds` function, passing in each of the 5 circles to the function.
...
2. Then, call the `game.checkCircleBounds(circle)` function.
